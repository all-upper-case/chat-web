...
export default function Settings({
  ...
  useEffect(() => {
    (async () => {
      ...
      // load system prompt (message idx 0) or fallback to saved settings
      if (conversationId) {
        ...
        setSystemPrompt(sys?.content || (data as SettingsRow | null)?.system_prompt || 'You are a helpful assistant.');
      } else {
        setSystemPrompt((data as SettingsRow | null)?.system_prompt || 'You are a helpful assistant.');
      }
    })();
  ...
  const saveSystemPrompt = async () => {
    ...
    if (conversationId) {
      ...
      if (m) await supabase.from('messages').update({ content: systemPrompt }).eq('id', m.id as string);
    }
    await supabase.from('settings').upsert({ user_id: auth.user.id, system_prompt: systemPrompt }, { onConflict: 'user_id' });
    onChanged();
  };
